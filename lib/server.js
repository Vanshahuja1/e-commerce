...
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
  ...
  <g fill="none" stroke="${fg}" stroke-width="2">
    <rect x="${size * 0.15}" y="${size * 0.2}" width="${size * 0.7}" height="${
    size * 0.6
  }" rx="${size * 0.04}"/>
    <circle cx="${size * 0.4}" cy="${size * 0.42}" r="${size * 0.08}"/>
    <path d="M ${size * 0.3} ${size * 0.6} L ${size * 0.45} ${size * 0.48} L ${
    size * 0.6
  } ${size * 0.62} L ${size * 0.7} ${size * 0.54}" />
  </g>
  ${
    text
      ? `<text x="50%" y="85%" text-anchor="middle" font-family="system-ui, -apple-system, Segoe UI, Roboto, Arial" font-size="${fontSize}" fill="${fg}">${text}</text>`
      : ""
  }
</svg>`;
...
async function sendOrderConfirmationEmail(user, order) {
  const itemsList = order.items
    .map(
      (item) =>
        `<li>${item.name} - Qty: ${item.quantity} - BHD${item.price}</li>`
    )
    .join("");
  const mailOptions = {
    from: process.env.EMAIL_USER,
...
app.get("/api/addresses", authenticateToken, async (req, res) => {
  try {
    const addresses = await Address.find({ userId: req.user.id })
      ...
      .limit(3); // Limit to 3 addresses as per frontend
    res.json({
      success: true,
      addresses,
    });
...
app.post("/api/addresses", authenticateToken, async (req, res) => {
  try {
    const { title, name, phone, address, city, state, pincode, isDefault } =
      req.body;
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const availableProducts = await Item.countDocuments({
        isAvailable: true,
      });
...
app.post(
  "/api/admin/seller-requests/:id/approve",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
...
app.post(
  "/api/admin/seller-requests/:id/reject",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
...
app.get("/api/admin-items", async (req, res) => {
  try {
    ...
    const items = await Item.find(query)
      .select(
        "name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt isSpecialItem"
      ) // include images, videoUrl
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `Item ${
          isAvailable ? "activated (visible)" : "deactivated (hidden)"
        } successfully`,
        item: {
          id: item._id,
          name: item.name,
  ...
  try {
    ...
    if (!user.isActive) {
      return res.status(403).json({
        success: false,
        message:
          "Your seller account is currently deactivated. Please contact admin.",
      });
    }
    const { name, productDetails, price, category, imageUrl, quantity, unit } =
      req.body;
    const item = new Item({
      name,
      productDetails,
...
app.get("/api/items", async (req, res) => {
  try {
    ...
    const query = {
      isAvailable: true,
      $or: [{ sellerId: { $in: activeSellerIds } }, { sellerName: "admin" }],
    };
    ...
    const items = await Item.find(query)
      .select(
        "name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt"
      ) // include images, videoUrl
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const items = await Item.find(query)
        .select(
          "name productDetails  price category imageUrl videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt isSpecialItem"
        ) // include videoUrl
        .sort({ createdAt: -1 })
        .limit(limit * 1)
        .skip((page - 1) * limit);
  ...
  async (req, res) => {
    try {
      ...
      const item = new Item({
        ...
        isAvailable: true,
        isSpecialItem: isSpecialItem === "true" || isSpecialItem === true,
      });
...
app.get("/api/items/my-items", authenticateToken, async (req, res) => {
  try {
    ...
    const items = await Item.find({ sellerId: user._id })
      .select(
        "name productDetails  price category imageUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt isSpecialItem"
      ) // Added discount, tax, hasVAT
      .sort({ createdAt: -1 });
...
app.post("/api/auth/facebook", async (req, res) => {
  try {
    ...
    if (!fbData.email) {
      return res
        .status(400)
        .json({ success: false, message: "Facebook account has no email" });
    }
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `Item ${
          isAvailable ? "made available" : "hidden"
        } successfully`,
        item,
      });
    } catch (error) {
...
app.get(
  "/api/admin/orders/:orderId/invoice/data",
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.files &&
        (req.files.images || req.files.video || req.files.media)
      ) {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log("Media URLs from request:", {
        imageUrl,
        imageUrls,
        videoUrl,
      });
      const imagesArr = Array.isArray(imageUrls)
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const items = await Item.find(query)
        .select(
          "name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName isAvailable createdAt updatedAt isSpecialItem"
        )
        .sort({ createdAt: -1 })
        .limit(limit * 1)
        .skip((page - 1) * limit);
...
app.put(
  ...
  async (req, res) => {
    try {
      const itemId = req.params.id;
      const updates = req.body;
      if ('isSpecialItem' in updates) {
       updates.isSpecialItem = updates.isSpecialItem === "true" || updates.isSpecialItem === true;
      }
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      doc
        ...
        .text(
          `Due Date: ${new Date(order.estimatedDelivery).toLocaleDateString()}`,
          50,
          180
        )
      ...
      doc.rect(50, tableTop, 500, 25).fillColor("#f3f4f6").fill();
