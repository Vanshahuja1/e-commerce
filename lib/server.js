...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const items = await Item.find(query)
        .select(
          "name productDetails  price category imageUrl videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt"
        ) // include videoUrl
        .sort({ createdAt: -1 })
        .limit(limit * 1)
        .skip((page - 1) * limit);
...
app.get("/api/items/my-items", authenticateToken, async (req, res) => {
  try {
    ...
    const items = await Item.find({ sellerId: user._id })
      .select(
        "name productDetails  price category imageUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt"
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const items = await Item.find(query)
        .select(
          "name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName isAvailable createdAt"
        )
        .sort({ createdAt: -1 })
        .limit(limit * 1)
        .skip((page - 1) * limit);
