...
// Serve general static assets from a public directory (optional)
// This won't throw if the directory is missing; it simply serves nothing.
app.use(express.static(path.join(__dirname, 'public')))

// Serve a dynamic placeholder SVG to prevent 404s like /placeholder.svg?thumb=1
app.get('/placeholder.svg', (req, res) => {
  const isThumb = req.query.thumb === '1' || req.query.thumb === 'true'
  const size = isThumb ? 64 : 256
  const bg = '#e5e7eb'   // Tailwind gray-200
  const fg = '#9ca3af'   // Tailwind gray-400
  const text = isThumb ? '' : 'No Image'
  const fontSize = Math.floor(size * 0.16)
  const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
  <rect width="100%" height="100%" fill="${bg}"/>
  <g fill="none" stroke="${fg}" stroke-width="2">
    <rect x="${size*0.15}" y="${size*0.2}" width="${size*0.7}" height="${size*0.6}" rx="${size*0.04}"/>
    <circle cx="${size*0.4}" cy="${size*0.42}" r="${size*0.08}"/>
    <path d="M ${size*0.3} ${size*0.6} L ${size*0.45} ${size*0.48} L ${size*0.6} ${size*0.62} L ${size*0.7} ${size*0.54}" />
  </g>
  ${text ? `<text x="50%" y="85%" text-anchor="middle" font-family="system-ui, -apple-system, Segoe UI, Roboto, Arial" font-size="${fontSize}" fill="${fg}">${text}</text>` : ''}
</svg>`
...
mongoose.connect(
  process.env.MONGODB_URI ||
    "mongodb+srv://pooja:pooja@cluster0.kxzink9.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0",
  {
...
const itemSchema = new mongoose.Schema({
  ...
  imageUrl: { type: String },
  images: [{ type: String, default: undefined }], // up to 4 images, keep imageUrl for backward compatibility
  ...
  discount: { type: Number, default: 0 }, // Discount percentage (0-100)
  tax: { type: Number, default: 0 },      // Tax percentage (0-100)
  hasVAT: { type: Boolean, default: false }, // VAT applicability (kept for backward compatibility)
})
...
app.get("/api/admin-items", async (req, res) => {
  try {
    ...
    const items = await Item.find(query)
      .select('name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt') // include images, videoUrl
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
...
app.post("/api/items", authenticateToken, async (req, res) => {
  try {
    ...
    const { name, productDetails , price, category, imageUrl, quantity, unit } = req.body
    const item = new Item({
      name,
      productDetails ,
      price,
...
app.get("/api/items", async (req, res) => {
  try {
    ...
    // Include items from active sellers OR admin-added items
    const query = {
      isAvailable: true,
      $or: [
...
app.get("/api/admin/items", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const items = await Item.find(query)
      .select('name productDetails  price category imageUrl videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt') // include videoUrl
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);
...
app.get("/api/admin-items", async (req, res) => {
  try {
    ...
    const items = await Item.find(query)
      .select('name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt') // Added images, videoUrl
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
...
app.post("/api/admin/items/json", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Validate required fields
    if (!name || !productDetails  || !price || !category || !quantity || !unit) {
      return res.status(400).json({
        success: false,
...
app.get("/api/items/my-items", authenticateToken, async (req, res) => {
  try {
    ...
    const items = await Item.find({ sellerId: user._id })
      .select('name productDetails  price category imageUrl quantity unit sellerId sellerName storeName discount tax hasVAT isAvailable createdAt updatedAt') // Added discount, tax, hasVAT
      .sort({ createdAt: -1 })
...
// On platforms with read-only filesystems (e.g., Render), write to OS temp dir
const tempUploadDir = path.join(os.tmpdir(), 'uploads', 'products');
const uploadDir = (process.env.NODE_ENV === 'production' || process.env.RENDER) ? tempUploadDir : 'uploads/products';
...
const upload = multer({ 
  storage: storage,
  limits: {
    // Increase limit to support typical mobile videos (up to ~50MB)
...
app.post("/api/admin/items", authenticateToken, requireAdmin, (req, res, next) => {
  ...
  if (contentType && contentType.includes('multipart/form-data')) {
    // Handle file upload
    // Accept up to 4 images as 'images', optional single 'video', and legacy single 'media'
    upload.fields([
      ...
      if (err) {
        console.error('Multer error on /api/admin/items:', err);
        const statusCode = err.code === 'LIMIT_FILE_SIZE' ? 413 : 400;
        return res.status(statusCode).json({ success: false, message: err.message || 'File upload error' });
      }
      next();
    });
  } else {
  ...
  try {
    ...
    const { 
      ...
      quantity, 
      unit, 
      imageUrl,  // from JSON requests
      imageUrls, // array from JSON requests
      videoUrl,  // from JSON requests
      discount = 0, 
      tax = 0, 
      hasVAT = false 
    } = req.body;
    ...
    // Collect media URLs (support up to 4 images + optional 1 video)
    const finalImages = [];
    let finalVideoUrl = '';

    // 1) From multipart uploads -> upload to Cloudinary
    if (req.files && (req.files.images || req.files.video || req.files.media)) {
      // Ensure Cloudinary is configured
      if (!cloudinary.config().cloud_name || !cloudinary.config().api_key || !cloudinary.config().api_secret) {
        return res.status(500).json({ success: false, message: 'Media upload service is not configured' });
      }
      const imageFiles = req.files.images || [];
      const legacyMedia = req.files.media || [];
      // If legacy media is an image, treat it as image; if video, as video
      legacyMedia.forEach(f => {
        if (f.mimetype && f.mimetype.startsWith('image/')) imageFiles.push(f);
      });
      const videoFiles = req.files.video || legacyMedia.filter(f => f.mimetype && f.mimetype.startsWith('video/'));
      ...
      if (videoFiles && videoFiles[0]) {
        ...
        const vRes = await cloudinary.uploader.upload(vfile.path, {
          folder: 'products',
          resource_type: 'video',
        });
        finalVideoUrl = vRes.secure_url;
        try { fs.unlinkSync(vfile.path); } catch (e) {}
      }
    }
    ...
    if (!finalImages.length) {
      if (Array.isArray(imageUrls)) {
        ...
        finalImages.push(...imageUrls.filter(Boolean));
      } else if (imageUrl) {
    ...
    console.log("Final media to save:", { images: finalImages, videoUrl: finalVideoUrl });

    // Create item with the determined media
    const item = new Item({
      name: name.trim(),
      productDetails : productDetails .trim(),
      price: parseFloat(price),
...
app.post("/api/admin/items/json", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    console.log("Media URLs from request:", { imageUrl, imageUrls, videoUrl });
    const imagesArr = Array.isArray(imageUrls) ? imageUrls.filter(Boolean) : (imageUrl ? [imageUrl] : []);
...
app.post("/api/admin/items/upload", authenticateToken, requireAdmin, upload.single('media'), async (req, res) => {
  try {
    ...
    const { name, productDetails , price, category, quantity, unit, discount = 0, tax = 0, hasVAT = false } = req.body;
    ...
    console.log("Generated media URL from upload:", { imageUrl, videoUrl });
...
app.get("/api/admin/items", authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const items = await Item.find(query)
      .select('name productDetails  price category imageUrl images videoUrl quantity unit sellerId sellerName isAvailable createdAt')
      .sort({ createdAt: -1 })
      .limit(limit * 1)
      .skip((page - 1) * limit);
